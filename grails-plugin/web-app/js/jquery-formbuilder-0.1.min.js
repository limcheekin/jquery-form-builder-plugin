...
"relative");$(".floatingPanel").css("top",$(".floatingPanelIdentifier").position)}});$("#paletteTabs").tabs({selected:this.options.tabSelected,disabled:this.options.tabDisabled});var a=this.options.widgets,c=a.length,b,e,d;for(d=0;d<c;d++){b=$.fb["fb"+a[d]].prototype.options;e=$('<a id="'+b._type+'" href="#" class="fbWidget">'+b.name+"</a>");e.button()["fb"+b._type]({fbOptions:this.options}).appendTo(b.belongsTo)}},_initBuilderPanel:function(){this._initFormSettings();if(this.options.readOnly){$("input:not(div.buttons input)").attr("disabled",
true);$("select").attr("disabled",true);$("textarea").attr("disabled",true)}else this._initSortableWidgets();this._initWidgetsEventBinder()},_initFormSettings:function(){var a=$(this.options._builderForm),c=$("#name",a),b=$("#description",a);a=$(this.options._formSettingsGeneralSection);$("input[id$='form.name']",a).val(c.val()).keyup(function(e){c.val($(e.target).val())});$("[id$='form.description']",a).val(b.val()).keyup(function(e){b.val($(e.target).val())})},_initWidgetsEventBinder:function(){var a=
...
name:function(a){var c=a.attr("rel"),b=$('<label for="field.name">Name (?)</label><br/>  \t\t\t\t  <input type="text" id="field.name" />');b.val(a.find("input[id$='fields["+c+"].name']").val()).keyup(function(e){a.find("input[id$='fields["+c+"].name']").val($(e.target).val()).change()});return b},getWidget:function(){$.fb.fbWidget.prototype._log("getWidget($this, settings, ctrlHolder) should be overriden by subclass.")},getFieldSettingsLanguageSection:function(){$.fb.fbWidget.prototype._log("getFieldSettingsLanguageSection($this, $widget, settings) should be overriden by subclass.")},
...
_init:function(){$.fb.fbWidget.prototype._init.call(this);this.options=$.extend({},$.fb.fbWidget.prototype.options,this.options);this._log("FbPlainText._create called. this.options.text = "+this.options.settings.en.text);this._log("FbPlainText._init called.")},destroy:function(){this._log("FbPlainText.destroy called.");$.fb.fbWidget.prototype.destroy.call(this)},getWidget:function(a,c,b){b.addClass(c.classes[1]);return $(a.options._html).text(c.text).addClass(c.classes[0])},getCounter:function(a){return $("div."+
a.options._type).size()+1},getFieldSettingsLanguageSection:function(a,c,b){var e=$('<label for="field.text">Text (?)</label><br />      <input type="text" id="field.text" />').val(c.find("div.PlainText").text()).keyup(function(){var g=$(this).val();c.find("div.PlainText").text(g);b.text=g;a.updateSettings(c,b)}),d=a.verticalAlignment().val(b.classes[1]).change(function(){var g=$(this).val();c.removeClass(b.classes[1]).addClass(g);b.classes[1]=g;a.updateSettings(c,b)}),f=a.horizontalAlignment().val(b.classes[0]).change(function(){var g=
c.find("div.PlainText"),h=$(this).val();g.removeClass(b.classes[0]).addClass(h);b.classes[0]=h;a.updateSettings(c,b)});return[a.oneColumn(e),a.twoColumns(d,f)]},getFieldSettingsGeneralSection:function(a,c){return[a.oneColumn(a.name(c))]}});$.widget("fb.fbPlainText",FbPlainText);
