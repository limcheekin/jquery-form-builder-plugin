...
j.removeClass(h.classes[0]).addClass(k);h.classes[0]=k;i._updateSettings(i)}).change();var o=a._fontStyles({names:["form.bold","form.italic","form.underline"],checked:h.fontStyles}).css("paddingLeft","3em");$("input[id$='form.bold']",o).change(function(){if($(this).attr("checked")){$(h.heading,j).css("fontWeight","bold");h.fontStyles[0]=1}else{$(h.heading,j).css("fontWeight","normal");h.fontStyles[0]=0}i._updateSettings(i)}).change();$("input[id$='form.italic']",o).change(function(){if($(this).attr("checked")){$(h.heading,
j).css("fontStyle","italic");h.fontStyles[1]=1}else{$(h.heading,j).css("fontStyle","normal");h.fontStyles[1]=0}i._updateSettings(i)}).change();$("input[id$='form.underline']",o).change(function(){if($(this).attr("checked")){$(h.heading,j).css("textDecoration","underline");h.fontStyles[2]=1}else{$(h.heading,j).css("textDecoration","none");h.fontStyles[2]=0}i._updateSettings(i)}).change();var p=g.settings.styles.fontFamily;if(p=="default")p=g._fontFamily;p=a._fontPicker({name:"form.fontFamily",value:p});
$("input[id$='form.fontFamily']",p).change(function(){var k=$(this).val();b.css("fontFamily",k);g.settings.styles.fontFamily=k;i._updateSettings(i)}).change();var m=g.settings.styles.fontSize;if(m=="default")m=g._fontSize;m=a._fontSize({label:"Size",name:"form.fontSize",value:m});$("select[id$='form.fontSize']",m).change(function(){var k=$(this).val();b.css("fontSize",k+"px");g.settings.styles.fontSize=k;i._updateSettings(i)}).change();p=a._twoColumns(p,m);p.find(".col2").addClass("noPaddingBottom").css("marginLeft",
"60%");m=g.settings.styles.color;if(m=="default")m=g._color;var s=g.settings.styles.backgroundColor;if(s=="default")s=g._backgroundColor;m=a._colorPanel({color:m,backgroundColor:s,idPrefix:"form."});$("input[id$='form.color']",m).change(function(){var k=$(this).data("colorPicker").color;b.css("color","#"+k);g.settings.styles.color=k;i._updateSettings(i)}).change();$("input[id$='form.backgroundColor']",m).change(function(){var k=$(this).data("colorPicker").color;i._log("background color change: value "+
k);b.css("backgroundColor","#"+k);g.settings.styles.backgroundColor=k;i._updateSettings(i)}).change();d.append(a._oneColumn(l)).append(a._twoRowsOneRow(n,r,o));e.append(a._fieldset({text:"Fonts"}).append(p)).append(m)},_languageChange:function(a){$.fb.formbuilder.prototype._log("languageChange("+$(this).val()+", "+$("option:selected",this).text()+")");var b=$.fb.fbWidget.prototype._getFbOptions(),c=$("."+$.fb.fbWidget.prototype.options._styleClass+":visible"),d=$(this).val(),e=$("option:selected",
this).text(),f=b.settings[d],g=$(".formHeading"),h=$(b._formSettingsLanguageSection),i,j,l,n;$("input[id$='form.name']",h).val(f.name);$("select[id$='form.heading'] option[value='"+f.heading+"']",h).attr("selected","true");var r=$("<"+f.heading+' class="heading">'+f.name+"</"+f.heading+">").css("fontWeight",f.fontStyles[0]==1?"bold":"normal").css("fontStyle",f.fontStyles[1]==1?"italic":"normal").css("textDecoration",f.fontStyles[2]==1?"underline":"none");$(".heading",g).replaceWith(r);$.fb.formbuilder.prototype._log("formSettings.fontStyles[2] = "+
f.fontStyles[2]);$("input[id$='form.bold']",h).attr("checked",f.fontStyles[0]);$("input[id$='form.italic']",h).attr("checked",f.fontStyles[1]);$("input[id$='form.underline']",h).attr("checked",f.fontStyles[2]);g.removeClass("leftAlign centerAlign rightAlign").addClass(f.classes[0]);$("select[id$='form.horizontalAlignment'] option[value='"+f.classes[0]+"']",h).attr("selected","true");c.each(function(){var o=$(this);(l=o.attr("class").indexOf($.fb.fbWidget.prototype.options._selectedClass)>-1)&&$(b._fieldSettingsLanguageSection+
" legend").text("Language: "+e);i=o.data("fbWidget");j=o.find("input[id$='fields["+o.attr("rel")+"].type']").val();$.fb.formbuilder.prototype._log("type = "+j);$this=$("#"+j).data("fb"+j);n={target:$this,item:o,settings:i[d]};n.item.selected=l;$this._languageChange(a,n)})},_updateSettings:function(a){a._log("_updateSettings = "+$.toJSON(a.options.settings));$("#settings").val($.toJSON(a.options.settings)).change()},_initWidgetsEventBinder:function(){var a=$("."+$.fb.fbWidget.prototype.options._styleClass);
if(a.size()>0){var b,c,d=["name","settings","sequence"];$(this.options._emptyBuilderPanel+":visible").hide();a.each(function(e){b=$(this);c=b.find("input[id$='fields["+e+"].type']").val();b.click($.fb["fb"+c].prototype._createFieldSettings);for(var f=0;f<d.length;f++)b.find("input[id$='fields["+e+"]."+d[f]+"']").change($.fb.fbWidget.prototype._updateStatus)});this.options.readOnly||a.find(".closeButton").click($.fb.fbWidget.prototype._deleteWidget)}},_initSortableWidgets:function(){var a=$(this.options._formControls);
...
"FF6600","FF6633","FF6666","FF6699","FF66CC","FF66FF","FF9900","FF9933","FF9966","FF9999","FF99CC","FF99FF","FFCC00","FFCC33","FFCC66","FFCC99","FFCCCC","FFCCFF","FFFF00","FFFF33","FFFF66","FFFF99","FFFFCC","FFFFFF"],customColors:[]},_init:function(){this._log("ColorPicker._init called.");this.element.append('<input type="text" id="'+this.options.name+'" name="'+this.options.name+'" /> \t\t\t\t<a class="floatLeft" href="#" rel="'+this.options.name+'"><img border="0" src="'+this.options.ico+'" alt="'+
this.options.openTxt+'"/></a>');var a=$("input",this.element).attr("readonly",true),b=this.element.data("colorPicker"),c="colorpicker_"+this.options.type;b.options.showColorCode?a.val(this.options.value):a.attr("disabled","true");a.data("colorPicker",{color:this.options.value});var d=$("#"+c);if(d.length==0){for(var e="",f=this.options[this.options.type+"Colors"],g,h=0;h<f.length;h++){g=f[h];if(g!="")e+='<li><a href="#" title="'+g+'" rel="'+g+'" style="background: #'+g+"; color: "+this._hexInvert(g)+
';">'+g+"</a></li>"}d=$('<div id="'+c+'" class="colorPicker">'+(this.options.title?"<h2>"+this.options.title+"</h2>":"")+"<ul>"+e+"</ul></div>").appendTo(document.body).hide();$(document.body).click(function(j){$(j.target).is(".colorPicker")||$(j.target).parents(".colorPicker").length||d.hide(b.options.speed)})}var i=$("a",this.element);if(this.options.inputBG){c=this.options.value;if(c.length==6)c="#"+c;a.css({background:c,color:"#"+this._hexInvert(c)})}i.click(function(){var j=i.offset();d.css({position:"absolute",
left:j.left+"px",top:j.top+"px",width:b.options.width+"px"}).show(b.options.speed).attr("rel",i.attr("rel"));return false});$("a",d).click(function(j){var l=$("input[id$='"+d.attr("rel")+"']"),n=$(this).attr("rel");$.fb.colorPicker.prototype._log("colorPicker. input id = "+d.attr("rel")+", hex = "+n+", title = "+$(this).attr("title")+", text = "+$(this).text());var r=l.parent().data("colorPicker").options;r.showColorCode&&l.val(n);l.data("colorPicker",{color:n});r.inputBG&&l.css({background:"#"+n,
color:"#"+b._hexInvert(n)});l.change();d.hide(b.options.speed);j.stopImmediatePropagation();return false})},_log:function(a){window.console&&window.console.log&&window.console.log(a)},_hexInvert:function(a){var b=a.substr(0,2),c=a.substr(2,2);a=a.substr(4,2);return 0.212671*b+0.71516*c+0.072169*a<0.5?"ffffff":"000000"}};$.widget("fb.colorPicker",ColorPicker);
...
"'\" onmouseout=\"this.style.backgroundColor='"+a.bgColor+'\'" style="font-family: '+d+';" value="'+d+'"> '+d.split(",")[0]+"</div>")});$("."+a.fontclass).click(function(c){var d=$(this),e=$("input[id$='"+d.parent().attr("rel")+"']"),f=d.attr("value");e.prev().text(d.text()).css("fontFamily",f);e.val(f).change();b.slideUp(a.speed);c.stopImmediatePropagation()})}};$.widget("fb.fontPicker",FontPicker);
...
title:"Basic Colors"}):b.colorPicker({name:name,value:value,ico:"../images/jquery.colourPicker.gif",title:"Web Safe Colors",type:"webSafe",width:360});return b},_fontPicker:function(a){a=$.extend({},a);this._log("fontPicker("+$.toJSON(a)+")");a.value=a.value.replace(/'/gi,"");a.value=a.value.split(",",1)[0];a.value=a.value!="default"?a.value:this._getFbOptions().settings.styles.fontFamily;if(!a.label)a.label="Font";var b=this._label(a).append('<div class="fontPicker" rel="'+a.name+'"></div>');$(".fontPicker",
b).fontPicker({name:a.name,defaultFont:a.value});return b},_fontSize:function(a){this._log("fontSize("+$.toJSON(a)+")");a.nobreak=true;var b=this._label(a).append('&nbsp;<select> \t\t    <option value="9">9</option> \t\t    <option value="10">10</option> \t\t    <option value="11">11</option> \t\t    <option value="12">12</option> \t\t    <option value="13">13</option> \t\t    <option value="14">14</option> \t\t    <option value="15">15</option> \t\t    <option value="16">16</option> \t\t    <option value="17">17</option> \t\t    <option value="18">18</option> \t\t    <option value="20">20</option> \t\t    <option value="22">22</option> \t\t    <option value="24">24</option> \t\t    <option value="28">28</option> \t\t    <option value="32">32</option> \t\t  </select>'),
